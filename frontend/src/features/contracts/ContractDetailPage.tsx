  ...
  return (
    <section className="admin-page contract-detail">
      <header className="detail-hero">
        ...
        <div><button disabled={isCancelled} title={isCancelled ? 'Hợp đồng đã hủy không thể đổi trạng thái.' : undefined} onClick={() => setModal('status')}>Đổi trạng thái</button><button disabled={isCancelled} title={isCancelled ? 'Hợp đồng đã hủy không thể ghi nhận thanh toán cũ.' : undefined} onClick={() => setModal('payment')}>Ghi nhận thanh toán cũ</button></div>
      </header>
      ...
      <section className="detail-card"><h2>Thanh toán ghi nhận nhanh (cũ)</h2><p className="helper-text">Mục này hiển thị các khoản PAY cũ trước Sprint 16 để đối chiếu. Luồng chính từ Sprint 16 là “Lịch thanh toán” bên dưới.</p><ContractPaymentTable items={contract.payments || []} onConfirm={(payment) => setModal(payment)} /></section>
      <section className="detail-card"><h2>Lịch thanh toán</h2><p className="helper-text">Đây là luồng thanh toán chính của Sprint 16. Summary bên dưới chỉ tính các lịch thanh toán PMT mới, không cộng các khoản PAY cũ.</p>{paymentSchedules.length === 0 ? <div className="form-warning">Chưa có lịch thanh toán Sprint 16 cho hợp đồng này. Số “Đã thanh toán/Còn lại” ở thẻ tổng quan đang lấy từ thanh toán PAY cũ nếu có.</div> : paymentSummary && <dl className="info-grid"><div><dt>Tổng phải thu theo lịch</dt><dd>{Number(paymentSummary.total_expected || 0).toLocaleString('vi-VN')}đ</dd></div><div><dt>Đã thu theo lịch</dt><dd>{Number(paymentSummary.total_paid || 0).toLocaleString('vi-VN')}đ</dd></div><div><dt>Còn lại theo lịch</dt><dd>{Number(paymentSummary.total_remaining || 0).toLocaleString('vi-VN')}đ</dd></div><div><dt>Quá hạn</dt><dd>{paymentSummary.overdue_count || 0}</dd></div></dl>}<PaymentScheduleForm contractId={contract.id} contractLabel={contract.contract_code} onSaved={load}/>{paymentSchedules.length > 0 && <table><thead><tr><th>Mã lịch</th><th>Đợt</th><th>Hạn</th><th>Phải thu</th><th>Đã thu</th><th>Còn lại</th><th>Trạng thái</th><th>Hành động</th></tr></thead><tbody>{paymentSchedules.map((p)=><tr key={p.id}><td>{p.payment_code}</td><td>{p.sequence_no}. {p.title}</td><td>{new Date(p.due_date).toLocaleDateString('vi-VN')}</td><td>{Number(p.expected_amount).toLocaleString('vi-VN')}đ</td><td>{Number(p.paid_amount).toLocaleString('vi-VN')}đ</td><td>{Number(p.remaining_amount).toLocaleString('vi-VN')}đ</td><td><PaymentBadge status={p.status}/></td><td><button onClick={() => navigateTo(`/payments/${p.id}`)}>Chi tiết</button></td></tr>)}</tbody></table>}</section>
