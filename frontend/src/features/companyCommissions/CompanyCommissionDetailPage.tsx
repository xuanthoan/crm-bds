  ...
  return (
    <section className="page">
      ...
      <div className="detail-grid">
        ...
        <section className="detail-card"><h2>Dòng thời gian nghiệp vụ</h2><ul className="commission-timeline-list">{(item.events || []).map((event) => <li key={event.id}><b>{dateTime(event.created_at)}</b> — {label(COMPANY_COMMISSION_EVENT_LABELS, event.event_type)}<br /><small>{event.reason || event.note || 'Không có ghi chú'}</small></li>)}</ul></section>
        <ActivityTimeline entityType="company_commission" entityId={receivableId} title="Lịch sử thao tác hệ thống" />
      </div>
    </section>
  );
}
