...
export function PaymentDetailPage({ paymentId }: { paymentId: string }) {
  ...
  return (
    <section className="admin-page">
      ...
      {item.status !== 'cancelled' && <InvoiceForm paymentId={item.id} onSaved={load}/>}<section className="detail-card"><h2>Phiếu thu</h2><table><thead><tr><th>Mã phiếu</th><th>Số tiền</th><th>Ngày thanh toán</th><th>Phương thức</th><th>Mã chứng từ</th><th>Trạng thái</th><th>Hành động</th></tr></thead><tbody>{(item.receipts || []).map((receipt) => <tr key={receipt.id}><td>{receipt.receipt_code}</td><td>{money(receipt.amount)}</td><td>{receipt.payment_date ? new Date(receipt.payment_date).toLocaleDateString('vi-VN') : 'Chưa cập nhật'}</td><td>{receipt.payment_method_label || 'Chưa cập nhật'}</td><td>{receipt.reference_no}</td><td>{receipt.status_label}</td><td>{receipt.status !== 'cancelled' && <button onClick={() => void cancelR(receipt.id)}>Hủy</button>}</td></tr>)}</tbody></table></section><section className="detail-card"><h2>Hóa đơn nháp</h2><ul>{(item.invoices || []).map((invoice) => <li key={invoice.id}>{invoice.invoice_code} · {money(invoice.amount)} · {invoice.status_label}</li>)}</ul></section>
    </section>
  );
}
