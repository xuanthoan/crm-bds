...
import { glossaryTerms, type GlossaryTerm } from './helpContent';
...
function groupGlossaryTerms(terms: GlossaryTerm[]): Record<string, GlossaryTerm[]> {
  return terms.reduce<Record<string, GlossaryTerm[]>>((grouped, term) => {
    grouped[term.group] = [...(grouped[term.group] || []), term];
    return grouped;
  }, {});
}
...
export function GlossaryPage() {
  ...
  const groupedTerms = useMemo(() => groupGlossaryTerms(terms), [terms]);
  ...
  return <section className="help-page glossary-page"><header className="page-header"><div><h1>Từ điển nghiệp vụ</h1><p>Giải thích các thuật ngữ thường gặp trong hệ thống CRM bất động sản.</p></div></header>
    ...
    {terms.length ? <div className="glossary-group-list">{Object.entries(groupedTerms).map(([groupName, groupTerms]) => <section className="glossary-group" key={groupName}><div className="glossary-group-header"><h2>{groupName}</h2><span>{groupTerms.length} thuật ngữ</span></div><div className="glossary-grid">{groupTerms.map((term) => <article className="glossary-card" key={term.term}><div className="glossary-card-heading"><h3>{term.term}</h3><span>{term.group}</span></div><p>{term.description}</p>{term.example && <p><strong>Ví dụ:</strong> {term.example}</p>}{term.modules?.length ? <small>Module liên quan: {term.modules.join(', ')}</small> : null}</article>)}</div></section>)}</div> : <p className="empty-state">Không tìm thấy thuật ngữ phù hợp.</p>}
  </section>;
}
