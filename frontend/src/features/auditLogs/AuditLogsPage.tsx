...
export function AuditLogsPage(){
 ...
 <section className="card audit-table-wrap">{error&&<p className="error-message">{error}</p>}<table><thead><tr><th>Thời gian</th><th>Người thao tác</th><th>Module</th><th>Hành động</th><th>Đối tượng</th><th>Mô tả</th><th>Lý do</th><th>Chi tiết</th></tr></thead><tbody>{items.map(i=><tr key={i.id}><td>{fmt(i.created_at)}</td><td><strong>{actorName(i)}</strong>{!isBlank(i.actor_email)&&actorName(i)!==i.actor_email?<><br/><small>{i.actor_email}</small></>:null}</td><td><span className="audit-module-badge">{displayModule(i)}</span></td><td><span className="audit-action-badge">{labelAction(i.action)}</span></td><td>{entityDisplay(i)}</td><td>{show(i.description)}</td><td>{show(i.reason)}</td><td><button className="secondary-button audit-detail-button" onClick={()=>setSelected(i)}>Xem chi tiết</button></td></tr>)}</tbody></table>{loading&&<p>Đang tải...</p>}<div className="audit-pagination" aria-label="Phân trang lịch sử thao tác"><div className="audit-pagination-controls"><button className="secondary-button audit-page-nav" disabled={loading||page<=1} onClick={()=>setPage(page-1)}>Trước</button>{paginationItems.map((item,index)=>item==='ellipsis'?<span key={`ellipsis-${index}`} className="audit-page-ellipsis">...</span>:<button key={item} type="button" className={`secondary-button audit-page-number${item===page?' active':''}`} aria-current={item===page?'page':undefined} disabled={loading||item===page} onClick={()=>setPage(item)}>{item}</button>)}<button className="secondary-button audit-page-nav" disabled={loading||page>=totalPages} onClick={()=>setPage(page+1)}>Sau</button></div><span className="audit-pagination-summary">Trang {page} / {totalPages} · {total} dòng</span></div></section>
