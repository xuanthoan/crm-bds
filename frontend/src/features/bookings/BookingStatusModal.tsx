...
export function BookingStatusModal({ booking, onClose, onSaved }: { booking: Booking; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title="Đổi trạng thái booking" onClose={onClose}>
    <form className="property-form" onSubmit={submit}>
      ...
      <label>Trạng thái mới<select value={status} disabled={isLocked || isFinal} onChange={(event) => selectStatus(event.target.value as BookingStatus)}>{statusOptions.map(([key, value]) => <option key={key} value={key}>{value}</option>)}</select></label><div className="workflow-hint-box full-span"><strong>Hướng dẫn nhanh</strong><ul><li><b>Mới tạo:</b> Booking vừa tạo, chưa giữ chỗ/chưa nhận tiền.</li><li><b>Giữ chỗ:</b> Khách đặt giữ chỗ tạm thời, chưa đặt cọc.</li><li><b>Đã cọc:</b> Đã nhận tiền cọc, BĐS được giữ cho khách.</li><li><b>Hết hạn giữ chỗ:</b> Khách không tiếp tục trong thời hạn giữ chỗ.</li><li><b>Đã hoàn tiền:</b> Đã hoàn toàn bộ hoặc một phần tiền; hệ thống có thể ghi nhận khấu trừ nếu có.</li><li><b>Đã hủy:</b> Booking không tiếp tục giao dịch.</li><li><b>Lưu ý:</b> Nếu đã có hợp đồng hiệu lực, hệ thống sẽ khóa thao tác booking.</li></ul></div>
