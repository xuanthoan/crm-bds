  ...
  return <section className="task-collaboration-panel" aria-label="Trao đổi công việc">
    ...
    {activeTab==='comments'&&<div className="task-tab-panel" role="tabpanel">
      ...
      <div className="task-comment-list">
        {comments.length?comments.map(c=><article key={c.id} className="task-comment-card"><div className="task-avatar">{initials(c.author?.full_name)}</div><div className="task-comment-body"><header><strong>{c.author?.full_name??'Người dùng'}</strong><small>{c.author?.email?`${c.author.email} · `:''}{dt(c.created_at)}</small></header><p className="comment-content"><LinkifiedText text={c.content}/></p></div></article>):!loading&&<div className="task-empty-state">Chưa có bình luận nào. Hãy ghi chú tiến độ xử lý công việc tại đây.</div>}
      </div>
