...
export type TaskFilters={page?:number;page_size?:number;q?:string;status?:string;task_type?:string;priority?:string;assigned_user_id?:string;due_from?:string;due_to?:string;related_customer_id?:string;related_booking_id?:string;related_deal_id?:string;related_contract_id?:string;overdue?:boolean;today?:boolean};
...
export function listTasks(filters:TaskFilters={}){const path=filters.today?'/api/v1/tasks/today':filters.overdue?`/api/v1/tasks/overdue?${qs(filters)}`:`/api/v1/tasks?${qs(filters)}`;return apiRequest<LeadTask[]>(path)}
