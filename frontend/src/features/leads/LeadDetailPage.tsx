...
export function LeadDetailPage({ leadId }: { leadId: string }) {
  const [lead, setLead] = useState<Lead | null>(null); const [owners, setOwners] = useState<AdminUser[]>([]); const [ownersLoading, setOwnersLoading] = useState(false); const [ownersError, setOwnersError] = useState<string[] | null>(null); const [errors, setErrors] = useState<string[] | null>(null); const [editing, setEditing] = useState(false); const [statusOpen, setStatusOpen] = useState(false); const [assignOpen, setAssignOpen] = useState(false); const [tasks,setTasks]=useState<LeadTask[]>([]); const [appointments,setAppointments]=useState<LeadAppointment[]>([]); const [taskOpen,setTaskOpen]=useState(false); const [appointmentOpen,setAppointmentOpen]=useState(false);
  ...
  return <section className="admin-page lead-detail"><button className="secondary-button back-button" onClick={() => navigateTo('/leads')}>← Quay lại danh sách</button><header className="detail-hero"><div><span className="lead-code">{lead.code}</span><h1>{lead.full_name}</h1><div><LeadStatusBadge status={lead.status}/><LeadPriorityBadge priority={lead.priority}/></div></div><div className="hero-actions">{canUpdate && <><button className="secondary-button" onClick={() => setEditing(true)}>Chỉnh sửa</button><button onClick={() => setStatusOpen(true)}>Đổi trạng thái</button></>}{canAssign && <button onClick={() => { setAssignOpen(true); void loadEligibleOwners(); }}>Chuyển lead</button>}{(can('leads.reclaim.team') || can('leads.reclaim.all')) && <button className="secondary-button" onClick={async () => { const reason = window.prompt('Lý do thu hồi lead', 'Sale không chăm sóc quá hạn'); if (!reason) return; try { await reclaimLead(leadId, undefined, reason); await load(); } catch (error) { setErrors(formatApiError(error)); } }}>Thu hồi</button>}</div></header><FormError messages={errors}/>
    ...
    <article className="detail-card"><header className="section-heading"><h2>Công việc</h2>{can('lead_tasks.create')&&<button onClick={()=>setTaskOpen(true)}>Tạo công việc</button>}</header><TaskTable items={tasks} onEdit={()=>navigateTo('/tasks')} onStatus={()=>navigateTo('/tasks')} onDelete={()=>navigateTo('/tasks')}/></article>
    <article className="detail-card"><header className="section-heading"><h2>Lịch hẹn</h2>{can('lead_appointments.create')&&<button onClick={()=>setAppointmentOpen(true)}>Tạo lịch hẹn</button>}</header><AppointmentTable items={appointments} onEdit={()=>navigateTo('/appointments')} onStatus={()=>navigateTo('/appointments')} onDelete={()=>navigateTo('/appointments')}/></article>
