...
import { listLeadScopeUsers } from '../organization/api';
import { navigateTo } from '../../routes/AppRoutes';
import { formatApiError } from '../../services/apiClient';
import { addLeadActivity, changeLeadStatus, getLead, reclaimLead, transferLead, updateLead } from './api';
...
export function LeadDetailPage({ leadId }: { leadId: string }) {
  ...
  useEffect(() => { void load(); void listLeadScopeUsers().then((r) => setOwners(r.data.filter((u) => u.roles.some((role) => SALES_ROLE_CODES.includes(role.code))))).catch(() => setOwners([])); }, [leadId]);
