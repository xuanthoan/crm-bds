...
import { assignLead, changeLeadStatus, createLead, deleteLead, listEligibleLeadAssignees, listLeads, updateLead, type LeadFilters as Filters } from './api';
...
export function LeadsPage() {
  const [leads, setLeads] = useState<Lead[]>([]); const [owners, setOwners] = useState<AdminUser[]>([]); const [ownersLoading, setOwnersLoading] = useState(false); const [ownersError, setOwnersError] = useState<string[] | null>(null); const [departments, setDepartments] = useState<Department[]>([]); const [teams, setTeams] = useState<Team[]>([]); const [filters, setFilters] = useState<Filters>({ page: 1, page_size: 20 }); const [meta, setMeta] = useState({ page: 1, total_pages: 0, total: 0 }); const [loading, setLoading] = useState(true); const [errors, setErrors] = useState<string[] | null>(null);
  ...
  useEffect(() => { void load(); if (canAssign) void loadEligibleOwners(); void Promise.all([listDepartments(), listTeams()]).then(([d,t]) => { setDepartments(d.data); setTeams(t.data); }).catch(() => { setDepartments([]); setTeams([]); }); }, []);
  ...
  return <section className="admin-page leads-page"><header className="page-header"><div><h1>Khách tiềm năng</h1><p>Quản lý và chăm sóc lead bất động sản theo phạm vi được phân quyền.</p></div>{can('leads.create') && <button onClick={() => setEditing(null)}>Tạo lead</button>}</header>
    <LeadFilters filters={filters} owners={owners} departments={departments} teams={teams} onChange={setFilters} onSearch={() => { const next = { ...filters, page: 1 }; setFilters(next); void load(next); }} /><FormError messages={errors}/>
