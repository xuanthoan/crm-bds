...
import {
  LEAD_ASSIGN_PERMISSIONS,
  LEAD_UPDATE_PERMISSIONS,
  SALES_ROLE_CODES,
  SOURCE_LABELS,
} from "./constants";
...
const money = (min: Lead["budget_min"], max: Lead["budget_max"]) =>
  !min && !max
    ? "—"
    : `${min ? Number(min).toLocaleString("vi-VN") : "0"} – ${max ? Number(max).toLocaleString("vi-VN") : "∞"}`;
...
export function LeadsPage() {
  ...
  const canUpdate = hasAny(LEAD_UPDATE_PERMISSIONS);
  const canAssign = hasAny(LEAD_ASSIGN_PERMISSIONS);
  async function load(next = filters) {
    setLoading(true);
    setErrors(null);
    try {
      const response = await listLeads(next);
      setLeads(response.data);
      setMeta(response.meta as typeof meta);
    } catch (error) {
      setErrors(formatApiError(error, "Không thể tải danh sách lead."));
    } finally {
      setLoading(false);
    }
  }
  async function loadEligibleOwners() {
    ...
    setOwnersError(null);
    try {
      const response = await listEligibleLeadAssignees();
      setOwners(
        response.data.filter((item) =>
          item.roles.some((role) => SALES_ROLE_CODES.includes(role.code)),
        ),
      );
    } catch (error) {
      setOwners([]);
      setOwnersError(
        formatApiError(
          error,
          "Không thể tải danh sách người phụ trách trong phạm vi được phân công.",
        ),
      );
    } finally {
    ...
    <section className="admin-page leads-page">
      <header className="page-header">
        <div>
          ...
          <p>
            Quản lý và chăm sóc lead bất động sản theo phạm vi được phân quyền.
          </p>
        </div>
        {can("leads.create") && (
          <button onClick={() => setEditing(null)}>Tạo lead</button>
        )}
      </header>
      <GuideBox
        title="Cách chăm sóc lead hiệu quả"
        items={[
          "Gọi lead mới càng sớm càng tốt.",
          "Sau mỗi lần gọi phải ghi chú kết quả.",
          "Nếu khách chưa quyết, tạo lịch chăm sóc tiếp theo.",
          "Cần khai thác nhu cầu, tài chính, thời gian mua và người quyết định.",
        ]}
      />
      <LeadFilters
        filters={filters}
        ...
        <table>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={10}>Chưa có lead phù hợp.</td>
              </tr>
            ) : (
              leads.map((lead) => (
                <tr key={lead.id}>
                  ...
                  <td>
                    ...
                    <small className="cell-subtitle">
                      {lead.phone_primary}
                      {lead.phone_secondary ? ` · ${lead.phone_secondary}` : ""}
                    </small>
                  </td>
                  <td>
                    {lead.source
                  ...
                  <td>{money(lead.budget_min, lead.budget_max)}</td>
                  <td>
                  ...
                  <td>{lead.owner?.full_name ?? "Chưa phân công"}</td>
                  <td>{dateTime(lead.next_follow_up_at)}</td>
      ...
      {statusLead && (
        <LeadStatusModal
          ...
          onSubmit={async (payload) => {
            await changeLeadStatus(statusLead.id, payload);
            setStatusLead(null);
            await load();
          }}
        />
      ...
      {assigning && (
        <LeadAssignModal
          ...
          onRetryOwners={() => void loadEligibleOwners()}
          onClose={() => setAssigning(null)}
          onSubmit={async (ownerId, note) => {
            await assignLead(assigning.id, ownerId, note);
            setAssigning(null);
            await load();
          }}
        />
      ...
      {deleting && (
        <ConfirmDialog
          title="Xóa lead"
          message={`Lead ${deleting.code} sẽ được xóa mềm và không còn xuất hiện trong danh sách.`}
