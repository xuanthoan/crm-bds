...
import { listDepartments, listLeadScopeUsers, listTeams } from '../organization/api';
...
export function LeadsPage() {
  const [leads, setLeads] = useState<Lead[]>([]); const [owners, setOwners] = useState<AdminUser[]>([]); const [departments, setDepartments] = useState<Department[]>([]); const [teams, setTeams] = useState<Team[]>([]); const [filters, setFilters] = useState<Filters>({ page: 1, page_size: 20 }); const [meta, setMeta] = useState({ page: 1, total_pages: 0, total: 0 }); const [loading, setLoading] = useState(true); const [errors, setErrors] = useState<string[] | null>(null);
  ...
  useEffect(() => { void load(); void Promise.all([listLeadScopeUsers(), listDepartments(), listTeams()]).then(([u,d,t]) => { setOwners(u.data.filter((item) => item.roles.some((role) => SALES_ROLE_CODES.includes(role.code)))); setDepartments(d.data); setTeams(t.data); }).catch(() => { setOwners([]); setDepartments([]); setTeams([]); }); }, []);
