...
export function LeadActivityForm({ onSubmit }: { onSubmit: (payload: { activity_type: LeadActivityType; title: string; content: string }) => Promise<void> }) {
  ...
  return <form className="activity-form detail-activity-form" onSubmit={submit}><div className="form-grid"><label>Loại hoạt động<select value={type} onChange={(e: any) => setType(e.target.value)}>{(['note','call','zalo','meeting'] as LeadActivityType[]).map((v) => <option key={v} value={v}>{ACTIVITY_LABELS[v]}</option>)}</select></label><label>Tiêu đề<input value={title} onChange={(e: any) => setTitle(e.target.value)} /></label><label className="full-span">Nội dung *<textarea rows={3} value={content} onChange={(e: any) => setContent(e.target.value)} required /></label></div><FormError messages={errors}/><button disabled={saving}>{saving ? 'Đang thêm…' : 'Thêm hoạt động'}</button></form>;
}
