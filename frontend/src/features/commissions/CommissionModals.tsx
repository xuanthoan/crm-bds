...
export function ActionModal({ type, commission, onClose, onSubmit }: { type: 'approve' | 'hold' | 'cancel' | 'paid'; commission: Commission; onClose: () => void; onSubmit: (p: Record<string, unknown>) => Promise<void> }) {
  ...
  return (
    <Modal title={titles[type]} onClose={onClose}>
      ...
      {type === 'paid' && <div className="form-warning"><div><b>Chính sách chi:</b> {commission.payout_policy?.payout_policy_label || commission.payout_policy_label || 'Chi theo hạn mức tiền hoa hồng công ty đã nhận'}</div><div><b>HH sale đã duyệt:</b> {money(commission.approved_commission)}</div><div><b>HH công ty xác nhận:</b> {money(commission.payout_policy?.company_commission_confirmed_receivable_amount || commission.company_commission_confirmed_receivable_amount || 0)}</div><div><b>Hoa hồng công ty đã nhận:</b> {money(commission.payout_policy?.company_commission_received_amount || commission.company_commission_received_amount || 0)}</div>{(commission.payout_policy?.payout_policy_code || commission.payout_policy_code) === 'received_ratio' && <div><b>Tỷ lệ đã thu:</b> {percent(commission.payout_policy?.company_commission_received_ratio ?? commission.company_commission_received_ratio)}</div>}<div><b>Tối đa có thể chi lần này:</b> {money(markPaidDefaultAmount)}</div></div>}
      ...
      <div className="commission-modal-form">
        ...
        {type === 'paid' && <><label>Ngày chi<input id="commission-payment-date" type="date" defaultValue={new Date().toISOString().slice(0,10)} /></label><label>Phương thức chi<select id="commission-payment-method" defaultValue="bank_transfer"><option value="cash">Tiền mặt</option><option value="bank_transfer">Chuyển khoản</option><option value="other">Khác</option></select></label><label className="full-span">Mã giao dịch/chứng từ<input id="commission-payment-reference" /></label><label className="full-span">Link chứng từ<input id="commission-attachment-url" /></label><label>Cách lưu phiếu<select id="commission-voucher-status" defaultValue="paid"><option value="draft">Lưu nháp</option><option value="paid">Xác nhận đã chi</option></select></label></>}
