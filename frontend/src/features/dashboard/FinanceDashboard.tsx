...
export function FinanceDashboard() {
  ...
  const cards:[string,string,string,(money?:boolean)=>string,boolean?][] = [
    ...
    ['Hoa hồng đã duyệt', 'commission_approved', 'Tổng hoa hồng được duyệt trong khoảng thời gian đã chọn, kể cả khoản đã chi sau đó.', () => `/commissions?approved_in_period=true&approved_from=${data.range.from_date}&approved_to=${data.range.to_date}`, true],
    ['Hoa hồng đã chi', 'commission_paid', 'Tổng hoa hồng đã chi trong khoảng thời gian đã chọn, gồm cả khoản chi một phần.', () => `/commissions?paid_in_period=true&paid_from=${data.range.from_date}&paid_to=${data.range.to_date}`, true],
