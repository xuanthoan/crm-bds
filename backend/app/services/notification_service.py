...
def list_notifications(db:Session,user:User,status=None,page=1,page_size=20):
    ...
    items=list(db.scalars(select(Notification).options(selectinload(Notification.related_task).load_only(Task.id,Task.task_code,Task.title)).where(*cond).order_by(Notification.created_at.desc()).offset((page-1)*page_size).limit(page_size)))
