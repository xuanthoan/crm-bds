...
SALE_DASHBOARD_ROLES = {"sale", "leader", "sales_manager", "admin", "director"}
ACTIVE_TASK_STATUSES = {"pending", "in_progress"}
ACTIVE_APPOINTMENT_STATUSES = {"scheduled", "rescheduled"}


def can_view_sale_dashboard(user: User) -> bool:
    ps = permissions(user); roles = _role_codes(user)
    return bool(user.is_superuser or SALE_DASHBOARD_PERMISSION in ps or "dashboard.view.own" in ps or roles & SALE_DASHBOARD_ROLES)


def _sale_profile(db: Session, user: User) -> dict:
    membership = db.scalar(select(UserOrganizationMembership).where(UserOrganizationMembership.user_id == user.id, UserOrganizationMembership.is_primary.is_(True)).limit(1)) or db.scalar(select(UserOrganizationMembership).where(UserOrganizationMembership.user_id == user.id).limit(1))
    team = db.get(Team, membership.team_id) if membership and membership.team_id else None
    dept = db.get(Department, membership.department_id) if membership and membership.department_id else None
    return {"user_id": str(user.id), "display_name": user.full_name, "team_id": str(team.id) if team else None, "team_name": team.name if team else None, "department_id": str(dept.id) if dept else None, "department_name": dept.name if dept else None}


def _sale_lead_item(lead):
    return {"id": str(lead.id), "title": lead.full_name, "subtitle": lead.phone_primary or lead.code, "status": lead.status, "status_label": lead.status, "due_at": lead.next_follow_up_at.isoformat() if lead.next_follow_up_at else None, "related_type": "lead", "related_id": str(lead.id), "open_url": f"/leads/{lead.id}", "phone": lead.phone_primary}


def _sale_task_item(task):
    return {"id": str(task.id), "title": task.title, "subtitle": task.lead.full_name if task.lead else "Công việc cá nhân", "status": task.status, "status_label": task.status, "due_at": task.due_at.isoformat() if task.due_at else None, "related_type": "task", "related_id": str(task.id), "open_url": f"/leads/{task.lead_id}" if task.lead_id else "/tasks"}


def _sale_appt_item(appt):
    return {"id": str(appt.id), "title": appt.title, "subtitle": appt.lead.full_name if appt.lead else (appt.appointment_type or "Lịch hẹn"), "status": appt.status, "status_label": appt.status, "due_at": appt.start_at.isoformat() if appt.start_at else None, "related_type": "appointment", "related_id": str(appt.id), "open_url": f"/leads/{appt.lead_id}" if appt.lead_id else "/appointments"}
...
def get_sale_dashboard(db: Session, user: User, preset: str | None = None, start_date: date | str | None = None, end_date: date | str | None = None) -> dict:
    if not can_view_sale_dashboard(user):
        from fastapi import HTTPException
        raise HTTPException(403, "Bạn không có quyền xem dashboard cá nhân.")
    r = resolve_dashboard_date_range(preset or "today", start_date, end_date); start, end = r["_start"], r["_end"]
    today_start, today_end = _bounds(); stamp = now(); stale_before = stamp - timedelta(days=STALE_LEAD_DAYS); uid = user.id
    lead_base = [Lead.deleted_at.is_(None), Lead.owner_id.__eq__(uid)]
    open_lead = Lead.status.notin_(CLOSED_LEAD_STATUSES)
    lead_new = _count(db, Lead, *lead_base, Lead.created_at >= start, Lead.created_at < end, Lead.duplicate_detected.is_(False), Lead.duplicate_of_customer_id.is_(None))
    lead_active = _count(db, Lead, *lead_base, open_lead)
    overdue_leads = _count(db, Lead, *lead_base, open_lead, Lead.next_follow_up_at.is_not(None), Lead.next_follow_up_at < stamp)
    due_today_leads = _count(db, Lead, *lead_base, open_lead, Lead.next_follow_up_at >= today_start, Lead.next_follow_up_at < today_end)
    no_activity = db.scalar(select(func.count()).select_from(Lead).where(*lead_base, open_lead, ~Lead.activities.any())) or 0
    stale = _count(db, Lead, *lead_base, open_lead, func.coalesce(Lead.last_contact_at, Lead.created_at) < stale_before)
    hot_leads = _count(db, Lead, *lead_base, open_lead, Lead.priority.in_({"high", "hot"}))
    customers = _count(db, Customer, Customer.deleted_at.is_(None), Customer.owner_id == uid, Customer.created_at >= start, Customer.created_at < end)
    task_today = _count(db, LeadTask, LeadTask.deleted_at.is_(None), LeadTask.assigned_to_id == uid, LeadTask.status.in_(ACTIVE_TASK_STATUSES), LeadTask.due_at >= today_start, LeadTask.due_at < today_end)
    task_overdue = _count(db, LeadTask, LeadTask.deleted_at.is_(None), LeadTask.assigned_to_id == uid, LeadTask.status.in_(ACTIVE_TASK_STATUSES), LeadTask.due_at < stamp)
    ...
    appt_overdue = _count(db, LeadAppointment, LeadAppointment.deleted_at.is_(None), LeadAppointment.assigned_to_id == uid, LeadAppointment.status.in_(ACTIVE_APPOINTMENT_STATUSES), LeadAppointment.start_at < stamp)
    ...
    leads_by_day = {d: {"date": d, "label": d[-2:], "count": 0} for d in days}; revenue_by_day = {d: {"date": d, "label": d[-2:], "amount": 0} for d in days}; tasks_done = {d: {"date": d, "label": d[-2:], "count": 0} for d in days}; activities = {d: {"date": d, "label": d[-2:], "count": 0} for d in days}
    for day, count in db.execute(select(func.date(Lead.created_at), func.count()).where(*lead_base, Lead.created_at >= start, Lead.created_at < end, Lead.duplicate_detected.is_(False), Lead.duplicate_of_customer_id.is_(None)).group_by(func.date(Lead.created_at))): leads_by_day[str(day)]["count"] = count
    for c, _d in contract_rows:
        key = (c.effective_date or c.signed_date or c.created_at).date().isoformat(); revenue_by_day[key]["amount"] += _money(c.contract_value)
    for day, count in db.execute(select(func.date(LeadTask.completed_at), func.count()).where(LeadTask.deleted_at.is_(None), LeadTask.assigned_to_id == uid, LeadTask.status == "completed", LeadTask.completed_at >= start, LeadTask.completed_at < end).group_by(func.date(LeadTask.completed_at))): tasks_done[str(day)]["count"] = count
    for day, count in db.execute(select(func.date(LeadActivity.created_at), func.count()).where(LeadActivity.user_id == uid, LeadActivity.created_at >= start, LeadActivity.created_at < end).group_by(func.date(LeadActivity.created_at))): activities[str(day)]["count"] = count
    summary = {"lead_new_count": lead_new, "lead_active_count": lead_active, "lead_overdue_care_count": overdue_leads, "lead_no_activity_count": no_activity, "lead_stale_count": stale, "customer_count": customers, "booking_count": bookings, "deposit_count": deposits, "deal_count": deals, "contract_signed_count": contract_count, "revenue_total": revenue, "customer_paid_total": customer_paid, "sales_commission_approved_total": sc_approved, "sales_commission_paid_total": sc_paid, "sales_commission_outstanding_total": max(sc_approved - sc_paid, 0), "sales_commission_payment_rate": sc_paid / sc_approved if sc_approved else None, "lead_to_customer_rate": _rate(customers, lead_new), "booking_to_contract_rate": _rate(contract_count, bookings)}
    return {"range": _range_public(r), "profile": _sale_profile(db, user), "today": {"tasks_due_today_count": task_today, "tasks_overdue_count": task_overdue, "appointments_today_count": appt_today, "appointments_overdue_count": appt_overdue, "leads_due_today_count": due_today_leads, "leads_overdue_care_count": overdue_leads, "hot_leads_count": hot_leads}, "summary": summary, "priority_actions": {"overdue_tasks": [_sale_task_item(x) for x in db.scalars(select(LeadTask).where(LeadTask.deleted_at.is_(None), LeadTask.assigned_to_id == uid, LeadTask.status.in_(ACTIVE_TASK_STATUSES), LeadTask.due_at < stamp).order_by(LeadTask.due_at).limit(10)).unique()], "today_tasks": [_sale_task_item(x) for x in db.scalars(select(LeadTask).where(LeadTask.deleted_at.is_(None), LeadTask.assigned_to_id == uid, LeadTask.status.in_(ACTIVE_TASK_STATUSES), LeadTask.due_at >= today_start, LeadTask.due_at < today_end).order_by(LeadTask.due_at).limit(10)).unique()], "today_appointments": [_sale_appt_item(x) for x in db.scalars(select(LeadAppointment).where(LeadAppointment.deleted_at.is_(None), LeadAppointment.assigned_to_id == uid, LeadAppointment.start_at >= today_start, LeadAppointment.start_at < today_end).order_by(LeadAppointment.start_at).limit(10)).unique()], "overdue_leads": [_sale_lead_item(x) for x in db.scalars(select(Lead).where(*lead_base, open_lead, Lead.next_follow_up_at.is_not(None), Lead.next_follow_up_at < stamp).order_by(Lead.next_follow_up_at).limit(10)).unique()], "hot_leads": [_sale_lead_item(x) for x in db.scalars(select(Lead).where(*lead_base, open_lead, Lead.priority.in_({"high", "hot"})).order_by(Lead.updated_at.desc()).limit(10)).unique()], "no_activity_leads": [_sale_lead_item(x) for x in db.scalars(select(Lead).where(*lead_base, open_lead, ~Lead.activities.any()).order_by(Lead.created_at).limit(10)).unique()], "stale_leads": [_sale_lead_item(x) for x in db.scalars(select(Lead).where(*lead_base, open_lead, func.coalesce(Lead.last_contact_at, Lead.created_at) < stale_before).order_by(func.coalesce(Lead.last_contact_at, Lead.created_at)).limit(10)).unique()]}, "pipeline": {"lead_count": lead_new, "customer_count": customers, "booking_count": bookings, "deposit_count": deposits, "deal_count": deals, "contract_count": contract_count, "lead_to_customer_rate": _rate(customers, lead_new), "booking_to_contract_rate": _rate(contract_count, bookings)}, "time_series": {"activities_by_day": list(activities.values()), "leads_by_day": list(leads_by_day.values()), "revenue_by_day": list(revenue_by_day.values()), "tasks_completed_by_day": list(tasks_done.values())}, "my_rank_hint": {"enabled": False, "message": "Xếp hạng cá nhân sẽ được bổ sung ở sprint sau."}}
