...
def create_project(db:Session,payload:ProjectCreate,actor:User)->Project:
    ...
    item=Project(project_code=_next_code(db),created_by_id=actor.id,**payload.model_dump(exclude_none=True)); db.add(item); db.flush()
