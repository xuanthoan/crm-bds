...
def list_appointments(db,user,page=1,page_size=20,**filters):
    ...
    if mine_only(filters.get("scope")):
        q=q.where(LeadAppointment.assigned_to_id==user.id)
    for key in ("status","appointment_type","assigned_to_id","lead_id"):
        if filters.get(key) is not None and not (key == "status" and filters.get(key) == "overdue"):
            q=q.where(getattr(LeadAppointment,key)==filters[key])
    ...
    if filters.get("status") == "overdue":
        q=q.where(LeadAppointment.status.in_({"scheduled","rescheduled"}), LeadAppointment.start_at < now())
