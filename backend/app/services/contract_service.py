...
def list_contracts(db,actor,page=1,page_size=20,q=None,status=None,contract_type=None,customer_id=None,property_unit_id=None,project_id=None,request_scope=None,date_from=None,date_to=None,payment_status=None):
    ...
    if status == "valid":
        conditions.append(Contract.status.in_({"signed", "effective", "active", "completed", "won"}))
        status = None
    if date_from:
        conditions.append(func.coalesce(Contract.effective_date, Contract.signed_date, Contract.created_at) >= datetime.combine(date_from, time.min, tzinfo=timezone.utc))
    if date_to:
        conditions.append(func.coalesce(Contract.effective_date, Contract.signed_date, Contract.created_at) <= datetime.combine(date_to, time.max, tzinfo=timezone.utc))
    if status: conditions.append(Contract.status.in_({x.strip() for x in status.split(',') if x.strip()}))
    if payment_status == "outstanding":
        confirmed = select(func.coalesce(func.sum(PaymentReceipt.amount), 0)).where(PaymentReceipt.contract_id == Contract.id, PaymentReceipt.deleted_at.is_(None), PaymentReceipt.status == "confirmed").scalar_subquery()
        conditions.append(Contract.contract_value > func.coalesce(Contract.deposit_value, 0) + confirmed)
...
def create_contract(db,payload:ContractCreate,actor):
    ...
    add_contract_activity(db,item,actor,"created",content=content,metadata=metadata); db.add(DealActivity(deal_id=deal.id,user_id=actor.id,activity_type="contract",title="Tạo hợp đồng từ giao dịch",content=content,metadata_json={"contract_id": str(item.id), "contract_code": item.contract_code})); _apply_status(db,item,actor,item.status); auto_task_for_contract_payment(db,item,actor); write_audit_log(db,action="contracts.create",user_id=actor.id,entity_type="contracts",entity_id=str(item.id)); db.commit(); db.refresh(item); return item
