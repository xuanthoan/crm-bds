...
def get_today_tasks(db,actor,page=1,page_size=200,q=None,assigned_user_id=None,with_meta=False):
    start_of_today,start_of_tomorrow=_today_bounds()
    all_items=list_tasks(db,actor,page=1,page_size=10000,q=q,assigned_user_id=assigned_user_id,due_from=start_of_today,due_before=start_of_tomorrow)[0]
...
def get_overdue_tasks(db,actor,page=1,page_size=200,q=None,assigned_user_id=None,with_meta=False):
    start_of_today,_=_today_bounds()
    # Keep overdue semantics fixed while allowing search/pagination for the dedicated page.
    all_items=list_tasks(db,actor,page=1,page_size=10000,q=q,assigned_user_id=assigned_user_id,due_before=start_of_today)[0]
    active=[t for t in all_items if t.status in {"open","in_progress"}]
    if not with_meta:
        return active[:page_size]
    total=len(active); start=(page-1)*page_size; end=start+page_size
