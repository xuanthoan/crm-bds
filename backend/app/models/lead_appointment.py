...
class LeadAppointment(Base):
    ...
    appointment_type: Mapped[str] = mapped_column(String(30)); status: Mapped[str] = mapped_column(String(30), default="scheduled", index=True)
    start_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), index=True); end_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    location: Mapped[str | None] = mapped_column(String(500)); meeting_link: Mapped[str | None] = mapped_column(String(1000))
    ...
    completed_by_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id")); completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    ...
    lead = relationship("Lead", back_populates="appointments"); assigned_to = relationship("User", foreign_keys=[assigned_to_id], lazy="joined")
