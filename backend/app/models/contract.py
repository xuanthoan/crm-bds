...
class Contract(Base):
    ...
    company_role: Mapped[str] = mapped_column(String(50), default="broker", nullable=False); actual_seller_type: Mapped[str | None] = mapped_column(String(50)); actual_seller_name: Mapped[str | None] = mapped_column(String(255)); commission_payer_type: Mapped[str | None] = mapped_column(String(50)); commission_payer_name: Mapped[str | None] = mapped_column(String(255)); brokerage_contract_code: Mapped[str | None] = mapped_column(String(100)); brokerage_policy_note: Mapped[str | None] = mapped_column(Text)
    ...
    payments=relationship("ContractPayment",back_populates="contract",order_by="ContractPayment.created_at.desc()",cascade="all, delete-orphan"); payment_schedules=relationship("PaymentSchedule",order_by="PaymentSchedule.created_at.desc()"); activities=relationship("ContractActivity",back_populates="contract",order_by="ContractActivity.created_at.desc()",cascade="all, delete-orphan")
