...
@router.get("")
def get_all(page:int=Query(1,ge=1),page_size:int=Query(20,ge=1,le=100),q:str|None=None,status_filter:str|None=Query(None,alias="status"),contract_type:str|None=None,customer_id:UUID|None=None,property_unit_id:UUID|None=None,project_id:UUID|None=None,scope:str|None=None,payment_status:str|None=None,date_from:date|None=None,date_to:date|None=None,db:Session=Depends(get_db),actor:User=Depends(require_auth)):
 items,meta=list_contracts(db,actor,page,page_size,q,status_filter,contract_type,customer_id,property_unit_id,project_id,scope,date_from,date_to,payment_status);return success_response([serialize_contract(i) for i in items],meta=meta)
