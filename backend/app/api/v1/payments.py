...
def receipt_list(page:int=Query(1,ge=1),page_size:int=Query(20,ge=1,le=100),q:str|None=None,status_filter:str|None=Query(None,alias='status'),scope:str|None=None,db:Session=Depends(get_db),actor:User=Depends(require_auth)):
    items,meta=list_all_receipts(db,actor,page,page_size,q,status_filter,scope); return success_response([serialize_receipt(i) for i in items],meta=meta)
...
@router.get('/invoices')
def invoice_list(page:int=Query(1,ge=1),page_size:int=Query(20,ge=1,le=100),q:str|None=None,status_filter:str|None=Query(None,alias='status'),scope:str|None=None,db:Session=Depends(get_db),actor:User=Depends(require_auth)):
