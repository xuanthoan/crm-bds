...
from app.services.dashboard_service import (
    BOSS_DASHBOARD_PERMISSION,
    get_boss_dashboard,
    get_my_work_summary,
    get_team_work_summary,
    get_sales_management_dashboard,
    get_sale_dashboard,
)
...
@router.get("/my-work")
def my_work(db: Session = Depends(get_db), user: User = Depends(require_auth)):
    return success_response(get_my_work_summary(db, user), "My work summary retrieved")

@router.get("/team-work")
@router.get("/leader")
def team_work(db: Session = Depends(get_db), user: User = Depends(require_auth)):
    return success_response(get_team_work_summary(db, user), "Team work summary retrieved")

@router.get("/sale")
def sale_dashboard(preset: str | None = None, start_date: str | None = None, end_date: str | None = None, from_date: str | None = None, to_date: str | None = None, db: Session = Depends(get_db), user: User = Depends(require_auth)):
    # GET /api/v1/dashboard/sale always uses current_user; never user_id/sale_id/team_id/department_id query ownership.
    try:
