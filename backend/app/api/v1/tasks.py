...
def today(page:int=Query(1,ge=1),page_size:int=Query(20,ge=1,le=100),q:str|None=None,scope:str|None=None,db:Session=Depends(get_db),actor:User=Depends(require_auth)):
    items,meta=get_today_tasks(db,actor,page=page,page_size=page_size,q=q,assigned_user_id=actor.id if scope=="mine" else None,with_meta=True); return success_response([serialize_task(i) for i in items],meta=meta)
@router.get("/overdue")
def overdue(page:int=Query(1,ge=1),page_size:int=Query(20,ge=1,le=100),q:str|None=None,scope:str|None=None,db:Session=Depends(get_db),actor:User=Depends(require_auth)):
    items,meta=get_overdue_tasks(db,actor,page=page,page_size=page_size,q=q,assigned_user_id=actor.id if scope=="mine" else None,with_meta=True); return success_response([serialize_task(i) for i in items],meta=meta)
