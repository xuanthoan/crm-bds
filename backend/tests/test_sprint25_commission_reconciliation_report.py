...
class Sprint25CommissionReconciliationReportSourceTest(unittest.TestCase):
    ...
    def test_frontend_route_menu_labels_and_no_raw_enums(self):
        ...
        self.assertIn('const secondaryFlags=', combined)
        self.assertIn('flag!==r.reconciliation_status', combined)
        self.assertIn('secondaryFlags(r).length>0', combined)
        self.assertIn('secondaryFlags(r).map', combined)
        self.assertNotIn('(r.reconciliation_flags ?? []).map', combined)
        self.assertIn('items.length===0', combined)
        self.assertIn('useLayoutEffect', combined)
        self.assertIn('topSpacerWidth', combined)
        self.assertIn('setTopSpacerWidth(tableScrollRef.current?.scrollWidth||0)', combined)
        self.assertIn('style={{width:topSpacerWidth}}', combined)
        self.assertNotIn('width: 140rem;\n}\n.commission-reconciliation-scroll', combined)
        self.assertIn('useRef<HTMLDivElement|null>', combined)
        self.assertIn('topScrollRef', combined)
        self.assertIn('tableScrollRef', combined)
        self.assertIn('syncTopScroll', combined)
        self.assertIn('syncTableScroll', combined)
        self.assertIn('commission-reconciliation-top-scroll', combined)
        self.assertIn('commission-reconciliation-scroll-spacer', combined)
