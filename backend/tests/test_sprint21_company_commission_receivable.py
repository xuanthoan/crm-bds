...
class Sprint21CompanyCommissionReceivableSourceTest(unittest.TestCase):
    ...
    def test_shared_commission_glossary_help_exists_on_both_pages(self):
        glossary=self.read('frontend/src/features/commissions/CommissionGlossaryHelp.tsx')
        company_page=self.read('frontend/src/features/companyCommissions/CompanyCommissionsPage.tsx')
        commissions_page=self.read('frontend/src/features/commissions/CommissionsPage.tsx')
        styles=self.read('frontend/src/styles.css')
        for text in ['CommissionGlossaryHelpButton', 'Phân biệt Hoa hồng công ty và Hoa hồng', 'Hoa hồng công ty = công ty đi thu', 'Hoa hồng = công ty đi trả', 'Chủ đầu tư trả công ty 40 triệu', 'Công ty trả sale 8 triệu']:
            self.assertIn(text, glossary + company_page + commissions_page)
        for text in ['page-title-with-help', 'commission-glossary-help-button', 'commission-glossary-table', 'commission-glossary-memory']:
            self.assertIn(text, styles + company_page + commissions_page)
