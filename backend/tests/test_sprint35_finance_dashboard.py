...
class Sprint35FinanceDashboardSourceTest(unittest.TestCase):
    def setUp(self):
        ...
        self.commissions = (ROOT / "backend/app/services/commission_service.py").read_text()
        self.commissions_api = (ROOT / "backend/app/api/v1/commissions.py").read_text()
    ...
    def test_due_and_outstanding_filters_share_live_schedule_rules(self):
        ...
        self.assertIn("from sqlalchemy import and_, func, or_, select", self.payments)
    ...
    def test_period_metrics_keep_matching_filter_fields(self):
        ...
        self.assertIn("approved_in_period=true&approved_from=${data.range.from_date}", self.finance_ui)
        self.assertIn("paid_in_period=true&paid_from=${data.range.from_date}", self.finance_ui)

    def test_commission_period_drilldowns_do_not_filter_by_current_status(self):
        self.assertIn("approved_in_period:bool|None=None,paid_in_period:bool|None=None", self.commissions_api)
        self.assertIn("special_period = f.get('approved_in_period') or f.get('paid_in_period')", self.commissions)
        self.assertIn("SalesCommission.approved_commission > 0", self.commissions)
        self.assertIn("SalesCommission.paid_amount > 0", self.commissions)
        self.assertIn("time.max if key.endswith('_to') else time.min", self.commissions)
