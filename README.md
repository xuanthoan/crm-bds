...
Sprint 4 introduces the first CRM business module while preserving authentication, RBAC, and the Sprint 3 administration screens.
...
The backend container runs the Sprint 4 Alembic migration automatically. To run it manually from `backend/`:
...
### Manual lead test checklist

1. Login as the default admin.
2. Open `/leads` and create a lead.
3. Create another lead with the same primary or secondary phone and confirm the API returns `Số điện thoại đã tồn tại trong hệ thống`.
4. Edit the lead's contact and interest information.
5. Change the status to `contacted` and confirm a status-change timeline item is created.
6. Assign the lead to an active Sale user and confirm an assignment timeline item is created.
7. Open the detail page and add a note, call, Zalo, or meeting activity.
8. Soft-delete the lead and confirm it no longer appears in the normal list.
9. Login as a Sale user and confirm only owned/created leads are visible.
10. Call `/api/v1/leads` without a token to verify 401, and with a user lacking lead view permission to verify 403.

> Known limitation: team and department lead scopes currently behave like own scope. Team/department hierarchy will be connected when organization structure is implemented in a later sprint.
